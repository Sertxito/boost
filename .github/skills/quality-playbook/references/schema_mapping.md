# Mapeo de tipos de schema (Paso 5b)

<!-- markdownlint-disable-file -->

Si el proyecto tiene una capa de validacion de schemas, debes entender que acepta cada campo antes de escribir pruebas de frontera. Capas comunes por lenguaje: modelos Pydantic (Python), JSON Schema (cualquiera), interfaces TypeScript/schemas Zod (TypeScript), anotaciones Bean Validation (Java), codecs de case class/decoders Circe (Scala), atributos serde (Rust). Sin este mapeo, escribirias mutaciones que el schema rechaza antes de llegar al codigo que intentas probar, produciendo errores de validacion en lugar de pruebas de frontera utiles.
...
## Reglas para elegir valores de mutacion
