...
description: "Explora cualquier codebase desde cero y genera seis artefactos de calidad: una constitución de calidad (QUALITY.md), pruebas funcionales trazadas a especificaciones, un protocolo de code review con generación de pruebas de regresión, un protocolo de pruebas de integración, una auditoría de especificaciones multimodelo (Council of Three) y un archivo de bootstrap para IA (AGENTS.md). Incluye análisis de completitud de máquinas de estado y detección de salvaguardas faltantes. Funciona con cualquier lenguaje (Python, Java, Scala, TypeScript, Go, Rust, etc.). Usa esta skill cuando el usuario pida configurar un quality playbook, generar pruebas funcionales desde especificaciones, crear una constitución de calidad, construir protocolos de testing, auditar código contra specs o establecer un sistema repetible de calidad para un proyecto. Actívala también cuando el usuario mencione 'quality playbook', 'spec audit', 'Council of Three', 'fitness-to-purpose', 'coverage theater' o quiera ir más allá de la generación básica de tests para construir un sistema de calidad completo basado en su codebase real."
...
# Generador de Quality Playbook

**Cuando esta skill inicie, muestra este banner antes de cualquier otra acción:**
...
Genera un sistema completo de calidad adaptado a un codebase específico. A diferencia de generadores de stubs de pruebas que trabajan de forma mecánica desde el código fuente, esta skill explora primero el proyecto (entendiendo dominio, arquitectura, especificaciones e historial de fallos) y luego produce un quality playbook basado en hallazgos reales.

## Por qué existe

La mayoría de los proyectos de software tienen tests, pero pocos tienen un *sistema* de calidad. Los tests verifican si el código funciona. Un sistema de calidad responde preguntas más difíciles: ¿qué significa "funcionar correctamente" para este proyecto específico? ¿De qué formas puede fallar que no quedarían cubiertas por tests? ¿Qué debería saber cada desarrollador (humano o IA) antes de tocar este código?

Sin un quality playbook, cada nueva contribución (y cada nueva sesión de IA) empieza desde cero: adivinando lo importante, escribiendo tests que parecen buenos pero no detectan bugs reales, y redescubriendo modos de fallo ya encontrados y corregidos meses atrás. Un quality playbook vuelve ese estándar explícito, persistente y heredable.

## Qué produce esta skill

Seis archivos que, en conjunto, forman un sistema de calidad repetible:

| Archivo | Proposito | Por que importa | ¿Ejecuta codigo? |
|------|---------|----------------|----------------|
| `quality/QUALITY.md` | Constitucion de calidad: objetivos de cobertura, escenarios de fitness-to-purpose, prevencion de teatro de cobertura | Toda sesion de IA lee esto primero. Define que significa "suficientemente bueno" para no adivinar. | No |
| `quality/test_functional.*` | Pruebas funcionales automatizadas derivadas de especificaciones | La red de seguridad. Pruebas vinculadas a lo que la especificacion dice que debe ocurrir, no solo a lo que hace el codigo. Usa el lenguaje del proyecto: `test_functional.py` (Python), `FunctionalSpec.scala` (Scala), `functional.test.ts` (TypeScript), `FunctionalTest.java` (Java), etc. | **Si** |
| `quality/RUN_CODE_REVIEW.md` | Protocolo de code review con guardrails que evitan hallazgos alucinados | Las revisiones de codigo de IA sin guardrails producen hallazgos seguros pero incorrectos. Los guardrails (lineas, grep antes de afirmar ausencia, leer cuerpos) mejoran la precision. | No |
| `quality/RUN_INTEGRATION_TESTS.md` | Protocolo de pruebas de integracion end-to-end para todas las variantes | Los unit tests pueden pasar, pero ¿el sistema funciona realmente end-to-end con servicios externos reales? | **Si** |
| `quality/RUN_SPEC_AUDIT.md` | Protocolo de auditoria de especificaciones multimodelo (Council of Three) | Ningun modelo de IA detecta todo. Tres modelos independientes con puntos ciegos distintos encuentran defectos que uno solo no detectaria. | No |
| `AGENTS.md` | Contexto de arranque para cualquier sesion de IA en este proyecto | El archivo de "lee esto primero". Sin el, las sesiones de IA pierden tiempo entendiendo el contexto. | No |

Plus output directories: `quality/code_reviews/`, `quality/spec_audits/`, `quality/results/`.

El entregable critico es el archivo de pruebas funcionales (nombrado segun el lenguaje y convenciones de testing del proyecto). Los protocolos en Markdown son documentacion para personas y agentes de IA. Las pruebas funcionales son la red de seguridad automatizada.

## Como usarla

Apunta esta skill a cualquier codebase:
...
Si ya existe un quality playbook (`quality/QUALITY.md`, pruebas funcionales, etc.), lee primero los archivos existentes y luego evaluarlos con los benchmarks de autocheck de la fase de verificacion. No asumas que esos archivos estan completos; tratalos como punto de partida.

---

## Fase 1: Explorar el codebase (aun no escribir)
...
## Fase 2: Generar el Quality Playbook
...
Si se crea desde cero: descripcion del proyecto, comandos de setup, comandos de build y test, resumen de arquitectura, decisiones clave de diseno, particularidades conocidas y enlaces a documentacion de calidad.
...
## Fase 4: Presentar, explorar, mejorar (interactiva)
