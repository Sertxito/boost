# Variables de pipeline, grupos de variables y agentes

<!-- markdownlint-disable-file -->

## Tabla de contenido

- [Variables de pipeline](#variables-de-pipeline)
- [Grupos de variables](#grupos-de-variables)
- [Carpetas de pipeline](#carpetas-de-pipeline)
- [Pools de agentes](#pools-de-agentes)
- [Colas de agentes](#colas-de-agentes)
- [Agentes](#agentes)

---

## Variables de pipeline

### Listar variables

```bash
az pipelines variable list --pipeline-id {pipeline-id}
```

### Crear variable

```bash
# Non-secret variable
az pipelines variable create \
  --name {var-name} \
  --value {var-value} \
  --pipeline-id {pipeline-id}

# Secret variable
az pipelines variable create \
  --name {var-name} \
  --secret true \
  --pipeline-id {pipeline-id}

# Secret with prompt
az pipelines variable create \
  --name {var-name} \
  --secret true \
  --prompt true \
  --pipeline-id {pipeline-id}
```

### Actualizar variable

```bash
az pipelines variable update \
  --name {var-name} \
  --value {new-value} \
  --pipeline-id {pipeline-id}

# Update secret variable
az pipelines variable update \
  --name {var-name} \
  --secret true \
  --value "{new-secret-value}" \
  --pipeline-id {pipeline-id}
```

### Eliminar variable

```bash
az pipelines variable delete --name {var-name} --pipeline-id {pipeline-id} --yes
```

## Grupos de variables

### Listar grupos de variables

```bash
az pipelines variable-group list
az pipelines variable-group list --output table
```

### Mostrar grupo de variables

```bash
az pipelines variable-group show --id {group-id}
```

### Crear grupo de variables

```bash
az pipelines variable-group create \
  --name {group-name} \
  --variables key1=value1 key2=value2 \
  --authorize true
```

### Actualizar grupo de variables

```bash
az pipelines variable-group update \
  --id {group-id} \
  --name {new-name} \
  --description "Updated description"
```

### Eliminar grupo de variables

```bash
az pipelines variable-group delete --id {group-id} --yes
```

### Variables del grupo de variables

```bash
# List variables
az pipelines variable-group variable list --group-id {group-id}

# Create non-secret variable
az pipelines variable-group variable create \
  --group-id {group-id} \
  --name {var-name} \
  --value {var-value}

# Create secret variable (will prompt for value if not provided)
az pipelines variable-group variable create \
  --group-id {group-id} \
  --name {var-name} \
  --secret true

# Create secret with environment variable
export AZURE_DEVOPS_EXT_PIPELINE_VAR_MySecret=secretvalue
az pipelines variable-group variable create \
  --group-id {group-id} \
  --name MySecret \
  --secret true

# Update variable
az pipelines variable-group variable update \
  --group-id {group-id} \
  --name {var-name} \
  --value {new-value} \
  --secret false

# Delete variable
az pipelines variable-group variable delete \
  --group-id {group-id} \
  --name {var-name}
```

## Carpetas de pipeline

### Listar carpetas

```bash
az pipelines folder list
```

### Crear carpeta

```bash
az pipelines folder create --path 'folder/subfolder' --description "My folder"
```

### Eliminar carpeta

```bash
az pipelines folder delete --path 'folder/subfolder'
```

### Actualizar carpeta

```bash
az pipelines folder update --path 'old-folder' --new-path 'new-folder'
```

## Pools de agentes

### Listar pools de agentes

```bash
az pipelines pool list
az pipelines pool list --pool-type automation
az pipelines pool list --pool-type deployment
```

### Mostrar pool de agentes

```bash
az pipelines pool show --pool-id {pool-id}
```

## Colas de agentes

### Listar colas de agentes

```bash
az pipelines queue list
az pipelines queue list --pool-name {pool-name}
```

### Mostrar cola de agentes

```bash
az pipelines queue show --id {queue-id}
```

## Agentes

### Listar agentes del pool

```bash
az pipelines agent list --pool-id {pool-id}
```

### Mostrar detalles del agente

```bash
az pipelines agent show --agent-id {agent-id} --pool-id {pool-id}
```